    ...
    <?php else: ?>
        <table class="border-separate border-spacing-0 text-center border border-b-2 border-gray-800 mx-auto">
            <thead class="sticky top-0 bg-indigo-500 text-white">
                <tr>
                    <th rowspan="2" class="border-2 border-r border-gray-800 px-3 w-48">Payment ID</th>
                    <th colspan="2" class="border-x border-y-2 border-gray-800 px-3">Reader</th>
                    <th rowspan="2" class="border-x border-y-2 border-gray-800 px-3 w-48">Card</th>
                    <th rowspan="2" class="border-x border-y-2 border-gray-800 px-3 w-48">Amount</th>
                    <th rowspan="2" class="border-2 border-l border-gray-800 px-3 w-48">Transaction Date</th>
                </tr>
                <tr>
                    <th class="border-x border-b-2 border-gray-800 px-3 w-48">Name</th>
                    <th class="border-x border-b-2 border-gray-800 px-3 w-48">Email</th>
                </tr>
            </thead>
                    ...
                    <?php foreach ($paymentData as $key => $data): ?>
                        <tr class="<?= $key % 2 == 0 ? 'bg-indigo-200' : 'bg-indigo-300'; ?>">
                            ...
                            <td class="border border-gray-800 p-2">
                                <?= $data['name'] ?></td>
                            <td class="border border-gray-800 p-2 max-w-[11rem] truncate"><a
                                    href="mailto:<?= $data['email'] ?>"><?= $data['email'] ?></a></td>
                            <td rowspan="1" class="border border-gray-800 p-2"><?= $data['card'] ?></td>
