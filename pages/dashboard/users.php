<header class="py-2.5 px-6">
    ...
    <div class="flex items-center gap-2">
        <form action="/formHandler" method="post" class="text-gray-800 divide-gray-500 relative w-[500px]">
            <input type="text" name="email" id="email" placeholder="Search readers by email"
                value="<?= $_COOKIE['data'] ?? ''; ?>" class="px-4 py-1 text-base outline-none w-full rounded-md">
            <button name="searchUser" class="absolute inset-y-0 right-0 px-2 rounded-r-md bg-slate-200"
                aria-label="Search">
                                    ...
                                    <table class="text-center border border-b-2 border-gray-800 border-separate border-spacing-0">
                                        ...
                                        <?php foreach ($rentedBooks as $key => $rentStatus): ?>
                                            ...
                                            <tr class="odd:bg-indigo-200 even:bg-indigo-300">
                                                ...
                                                ?>
                                                <td class="border border-gray-800 p-2">
                                                    &#x20B9;<?= $fine = $dueDate < $currentDate ? ($days * $bookData['fine']) : 0 ?></td>
                                                <td class="border border-r-2 border-gray-800 p-2">&#x20B9;<?= $base + $additional + $fine ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </table>
                                <?php else: ?>
                                    <h3 class="text-3xl font-medium text-gray-600"><?= $user['name']; ?> has not taken any book on rent.
                                    </h3>
                                <?php endif; ?>
                            </div>
                        </article>
...
<?php else: ?>
    ...
    $user = $query->selectOne('users', $uuid, 'uuid');
    $query = new DatabaseQuery();
    $rentedBooks = $query->selectAllSpecific('rented_books', $user['uuid'], 'user_id');
    ?>
    <section class="bg-white rounded-md p-2 h-fit">
        <article class="space-y-3">
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-2xl"><?= $user['name']; ?></h2>
                <?php if (!$user['role']): ?>
            ...
            <div class="flex items-center justify-between gap-12">
                ...
                <button type="button" class="bg-indigo-500 text-white px-4 py-1 rounded-md font-medium hover:bg-indigo-700"
                        ...
                        <table class="text-center border border-b-2 border-gray-800 border-separate border-spacing-0">
                            <thead class="sticky top-0 bg-indigo-500 text-white">
                                ...
                                <tr>
                                    <th class="border-x border-b-2 border-gray-800 px-1">Base Price</th>
                                    <th class="border-x border-b-2 border-gray-800 px-1">Rent after 30 days</th>
                                    <th class="border-x border-b-2 border-gray-800 px-1">Fine</th>
                                    <th class="border-x border-b-2 border-r-2 border-gray-800 px-1">Total Rent</th>
                                </tr>
                            </thead>
                            <tbody>
