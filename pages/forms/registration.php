...
<main class="container border-x space-y-6 py-8 min-h-[calc(100vh-4rem-3.5rem)]">
    <h1 class="text-center text-4xl font-semibold">
        <?= empty($_SERVER['QUERY_STRING']) ? ($uri !== 'addMember' ? 'New User Registration' : 'New Admin Registration') : 'Update Your Info'; ?></h1>
    <form action="/formHandler" method="post" enctype="multipart/form-data" class="space-y-8 max-w-md mx-auto">
        ...
        <div class="grid grid-cols-2 gap-6">
            <div>
                <input type="password" name="<?= empty($_SERVER['QUERY_STRING']) ? 'password' : 'oldPassword' ?>" id="password" placeholder="<?= empty($_SERVER['QUERY_STRING']) ? 'Password' : 'Old Password' ?>"
                    class="w-full px-4 py-2 border border-gray-600 rounded outline-indigo-600 placeholder:text-gray-500">
                <span class="text-red-600 text-sm font-medium"><?= empty($_SERVER['QUERY_STRING']) ? ($err['passwordErr'] ?? '') : ($err['cnfrmPasswordErr'] ?? ''); ?></span>
            </div>
            <div>
                <input type="password" name="<?= empty($_SERVER['QUERY_STRING']) ? 'confirmPassword' : 'password' ?>" id="confirmPassword" placeholder="<?= empty($_SERVER['QUERY_STRING']) ? 'Confirm Password' : 'New Password' ?>"
                    class="w-full px-4 py-2 border border-gray-600 rounded outline-indigo-600 placeholder:text-gray-500">
                <span class="text-red-600 text-sm font-medium"><?= empty($_SERVER['QUERY_STRING']) ? ($err['cnfrmPasswordErr'] ?? '') : ($err['passwordErr'] ?? ''); ?></span>
            </div>
        </div>
        <input type="hidden" name="id" value="<?= $_SERVER['QUERY_STRING']; ?>">
        <button name="<?= empty($_SERVER['QUERY_STRING']) ? ($uri !== 'addMember' ? 'register' : 'registerAdmin') : 'updateData'; ?>" id="<?= empty($_SERVER['QUERY_STRING']) ? 'register' : 'updateData' ?>"
